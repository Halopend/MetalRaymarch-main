...
// Runs same scene with different iteration counts to measure scaling
// ============================================================================

kernel void iterationScalingKernel(
    texture2d<float, access::write> outputTexture [[texture(0)]],
    constant DiagnosticUniforms &uniforms [[buffer(0)]],
    uint2 gid [[thread_position_in_grid]]
) {
    ...
    // Use iteration count from forceIterations
    int iters = max(1, uniforms.forceIterations);
    int steps = uniforms.maxRaySteps;
    ...
    for (int i = 0; i < steps; i++) {
        ...
        if (d < 0.0001 || t > 50.0) break;
        t += d * 0.9;
    }
    
    // Encode step count as grayscale
    float normalized = float(hitSteps) / float(steps);
    outputTexture.write(float4(normalized, normalized, normalized, 1.0), gid);
}

// ============================================================================
// RAY STEP SCALING TEST
// Measures how performance changes with different max ray step counts
// ============================================================================

kernel void rayStepScalingKernel(
    texture2d<float, access::write> outputTexture [[texture(0)]],
    constant DiagnosticUniforms &uniforms [[buffer(0)]],
    uint2 gid [[thread_position_in_grid]]
) {
    ...
    int iters = uniforms.fractalIterations;
    int steps = max(4, uniforms.forceRaySteps);  // Use override
    ...
    // R = hit, G = steps used, B = distance
    outputTexture.write(float4(hit ? 1.0 : 0.0, float(hitSteps) / float(steps), saturate(t / 50.0), 1.0), gid);
}
