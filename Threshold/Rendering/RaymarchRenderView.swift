...
private final class ThresholdMacRenderer {
    ...
    func draw(appModel: AppModel) {
        ...
        commandBuffer.addCompletedHandler { [inFlightSemaphore, renderLogger, gpuErrorRecovery] cb in
            if cb.status == .error {
    ...
    private func updateAudioLevels(appModel: AppModel,
        ...
        let bassRaw    = analyzer.bassLevel
        let midRaw     = analyzer.midLevel
        let trebleRaw  = analyzer.trebleLevel
        let peakRaw    = analyzer.peakLevel
        let levelRaw   = analyzer.level

        guard bassRaw.isFinite, midRaw.isFinite, trebleRaw.isFinite,
              peakRaw.isFinite, levelRaw.isFinite else {
            renderLogger.error("Non-finite analyzer level detected — zeroing audio inputs (bass=\(bassRaw) mid=\(midRaw) treble=\(trebleRaw) peak=\(peakRaw) level=\(levelRaw))")
            settings.bassLevel = 0
            settings.midLevel = 0
            settings.trebleLevel = 0
            settings.beatIntensity = 0
        ...
        settings.bassLevel = min(1.0, bassRaw * bassSensitivity)
        settings.midLevel = min(1.0, midRaw * midSensitivity)
        settings.trebleLevel = min(1.0, trebleRaw * trebleSensitivity)
        settings.beatIntensity = min(1.0, peakRaw * 0.7 * beatSensitivity)
        settings.audioLevel = levelRaw
    }
