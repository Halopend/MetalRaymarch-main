...
#include "KaleidoIFS/KaleidoIFS.h"
...
FORCE_INLINE float FractalDE_Dispatch(float3 pos, int fractalType, FormulaParams fp, int iterations) {
    switch (fractalType) {
        ...
        case FractalTypeKaleidoIFS:
            return DE_KaleidoIFS_Dist(pos, fp, fp.rotMatrix1, iterations);
        default:
...
FORCE_INLINE float FractalDE_WithOrbit(float3 pos, int fractalType, FormulaParams fp,
    ...
    switch (fractalType) {
        ...
        case FractalTypeKaleidoIFS:
            return DE_KaleidoIFS(pos, fp, fp.rotMatrix1, iterations, colorIterations, orbit);
        default:
            orbit.trap = 1e20f;
