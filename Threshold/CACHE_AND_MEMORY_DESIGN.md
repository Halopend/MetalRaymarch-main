...
**Already diverged:** Mac sets `warmStartEnabled:0` + identity `previousViewProj` (warm-start is visionOS-only), bakes Halton jitter into the projection, and uses a clamped divisor `viewDistanceScale = max(min(effectiveScale, smoothedScale), floor)` (`:1316`) that the visionOS copy lacks (`:118` divides by `traceScale` directly).
...
### 3.5 Dual MetalFX managers — do NOT merge (verified non-opportunity)

Three scaler managers exist: visionOS `MetalFXManager` (type2DArray / per-eye views + ping-pong depth for temporal warm-start) and Mac/iOS `MacSpatialUpscaler` + `MacTemporalUpscaler` (plain 2D). `MacSpatialUpscaler.swift:5–7` documents *why* it does not reuse the visionOS manager. The genuinely shared piece — texture allocation with the iOS `.memoryless` depth rule — is already extracted into `MetalFXTextureSupport`. The stereo-array-vs-2D split is a real API-shape difference; merging would add conditional branches for a handful of saved lines. **No action.** The only flag: `MetalFXManager.swift:47` redeclares `minimumInputShortEdge=128` independently of `MetalFXTextureSupport.minimumInputShortEdge`, but that file is `#if os(macOS) || os(iOS)` and compiled *out* on visionOS, so sourcing the constant from it would first require widening the availability guard — not free. Treat as completeness flag, not an action item.

---

## 4. Right kinds of memory

**Baseline is already disciplined:** a repo-wide grep for `.managed`/`storageModeManaged` returns **zero hits** (correct for Apple-Silicon-only targets); all per-frame CPU-written buffers are `.shared`; GPU-only compute textures/depths are `.private`; the iOS `.memoryless` transient-depth rule is centralized in `MetalFXTextureSupport`. The MetalFX output deliberately drops `.shaderWrite` to enable lossless framebuffer compression (`MetalFXManager.swift:271–277`) — a correct, preserve-on-future-edit bandwidth choice. The wins below are narrow.
...
| visionOS render-target cluster | `Renderer.swift:1507–1589`, `MetalFXManager.swift:220–310` | individual `makeTexture`, no heap | **investigate, do NOT commit** — see §4.2 | Low confidence; blocked by compression. |
| visionOS render targets / depths | `Renderer.swift:1528/1570` | `.private` | **keep** | Correct GPU-only. |
| MetalFX temporal/manager depth | `MacTemporalUpscaler.swift:197`, `MetalFXManager.swift:250` | `.private` (`.shaderRead`) | **keep — must NOT force memoryless** | MetalFX/ASW *samples* depth, so `.shaderRead` is mandatory and incompatible with tile-only `.memoryless`. Document inline. |
...
### 4.2 visionOS render-target cluster heap + aliasing (LOW confidence — investigate, don't commit)

The compute-output / temporal-depth set and the MetalFX input/depth/output set have non-overlapping per-frame lifetimes (`renderWithAdaptiveCompute` commits and returns early at `Renderer.swift:1072`). In principle a placement `MTLHeap` with `makeAliasable` could reclaim ~one eye-resolution texture. **But three premises are weak:** (1) MetalFX *input* and *depth* are sized at reduced `inputWidth/Height` (× `resolutionScale`), not eye resolution — only the *output* is full-res, so the footprint headline is smaller than claimed; (2) MetalFX depth (input-res) and temporal depth (drawable-res) are different sizes, so that alias can't reclaim cleanly — only MetalFX-output ↔ compute-output have matching dimensions; (3) **that one viable alias directly collides with the lossless-compression optimization** (`MetalFXManager.swift:271–277` dropped `.shaderWrite` from the output to enable compression; compute output *requires* `.shaderWrite`, so a shared backing forfeits the compression win). `makeAliasable` must fire only at mode/resolution boundaries (per-frame would corrupt in-flight work). **Net: medium risk, low confidence — flag for on-device memory-pressure measurement before any build; likely not worth it. Do not implement speculatively.**
...
- **`MTLResidencySet` `initialCapacity=8`** (`RendererSetupAndSession.swift:23`): steady-state max is exactly 2 base buffers + 3 compute textures + 3 MetalFX textures = **exactly 8** (not "past 8"). Whether `MTLResidencySet` reallocs *at* vs *above* capacity is unspecified by Apple, so "forces a grow" is not guaranteed. Bump to ~10–12 for headroom regardless — a single literal, zero risk.
...
- **B2. `MTLBinaryArchive` for built-in init-path + preset pipelines** (§2.1/§2.3, HIGH / medium). ✅ **DONE for the compute path (2026-06-26)** — `PipelineBinaryArchive` wired into the visionOS compute builder + startup batch + lazy builds (see §2.1 note). **This is the actual lead of Slice B** — it was promoted over B1 because B1-as-written turned out not to be a real API (see B1 below). **Remaining:** the render path (visionOS `buildRenderPipelineWithDevice` init batch is a clean `descriptor.binaryArchives` drop-in; Mac `RaymarchRenderView` pipelines) and **archiving the preset descriptors** (`precompilePresetPipelines` builds render PSOs, not yet archived).
...
- `/Users/halopend/Documents/GitHub/Polinate/TEMP/MetalRaymarch-main/Threshold/Rendering/MetalFXManager.swift`
