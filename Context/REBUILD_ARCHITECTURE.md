...
> **Status (2026-07-04): PROPOSED — no go/no-go decision recorded yet.**
> Registered as TECH_DEBT.md #16; arch items #10–13 there are parked pending this call.
> Decide when ready and record it here (proceed / park / criteria). Suggested decision
> inputs: (a) does PERF_PUSH Phase 2 (deferred shading) land acceptably in the current
> tree, or does the megakernel resist it? (b) does the next parameter-heavy feature
> spend most of its cost re-wiring UI/persistence/binding (the catalog's whole pitch)?
> (c) Vision Pro perf baseline — a rebuild is the natural place for the compute-path
> redesign if the current path can't hold 90 fps. Regardless of the decision, the
> seam-hardening register items (#1 embed test, #2 Uniforms builder, #3 layering tests,
> #5 CI) stay worth doing — the tests double as behavior pins for any migration.
