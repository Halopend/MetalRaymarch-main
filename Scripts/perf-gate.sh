...
# build.sh builds into the repo-local DerivedData (.build/DerivedData, or
# THRESHOLD_DERIVED_DATA_PATH). Prefer that; fall back to Xcode's global
# DerivedData resolved by WorkspacePath — "newest Threshold-*" is wrong
# whenever a worktree/second checkout has built more recently.
APP="${THRESHOLD_DERIVED_DATA_PATH:-$REPO/.build/DerivedData}/Build/Products/Debug/Threshold.app"
if [[ ! -x "$APP/Contents/MacOS/Threshold" ]]; then
    APP=""
