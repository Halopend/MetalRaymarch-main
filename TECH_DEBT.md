...
| 21 | ~~**`FunctionConstantIndex` Mac literal lockstep**~~ | Code | – | – | – | – | **DONE 2026-07-25**: the enum moved to dependency-free `Threshold/Rendering/Core/FunctionConstants.swift` (no imports, so the synchronized group compiles it into all three app targets); the Mac/iOS specialization path (`ViewportSpecializedPipelineBuilder.build`) now sets all 11 constants via `.rawValue` and the bare-literal mirror + comment are gone. `FragmentTextureIndex`/`BufferIndex` checked and NOT similarly trapped (`FragmentTextureIndex` is set only by the visionOS `Renderer`; `BufferIndex` bridges from `ShaderTypes.h` into every target). Verified: `build.sh all` + `perf-gate.sh` byte-identical PNG (no index drifted). |
