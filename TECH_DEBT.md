...
> `makeUniforms`; ⚠️ verify with `build.sh test` + `ql_render_check.sh` before
> treating as done). **#8d superseded:** the by-value occupancy concern was
...
| 2 | ~~**Single `Uniforms` builder**~~ | Code | – | – | – | – | **DONE 2026-07-09** (⚠️ pending build/QL verification): `Threshold/Rendering/Core/UniformsBuilder.swift` holds one `makeUniforms(settings:effectiveScale:time:platform:)` + a `UniformsPlatformInputs` struct. All three sites (`RendererGameState`, Mac `RaymarchRenderView.makeUniforms`, QL `HeadlessRenderer.packUniforms`) now assemble via it, so the ~76-field list + the duplicated derived math (bubble scaling, epsilon/LOD, spring, animated color/glow) live once; platform-divergent fields are named on the struct (compile-checked). Added to the QL `SHARED_SOURCES`. **Must pass `build.sh test` + `ql_render_check.sh` (byte-exact) before commit.** |
